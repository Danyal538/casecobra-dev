...
const DesignConfigurator = ({
  ...
  const [rederedDimension, setRenderedDimension] = useState({
  ...
  async function saveConfiguration() {
    try {
      ...
      ctx?.drawImage(
        ...
        actualX,
        actualY,
        rederedDimension.height,
        rederedDimension.width,
      )
      const base64 = canvas.toDataURL();
      console.log(base64);
      const base64Data = base64.split(',')[1];

      const blob = blobToImage(base64Data, "image/png")
      const file = new File([blob], "filename.png", { type: "image/png" });
      await startUpload([file], { configId })
    } catch (err) {
      toast.error("Something went wrong. Problem in saving your config, please try again");
      console.log(err);
    }
  }

  function blobToImage(base64: string, mimeType: string) {
  ...
  return (
    <div className="relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20">
      ...
      <div className="h-[37.5rem] w-full col-span-full lg:col-span-1 flex flex-col bg-white">
        <ScrollArea className="relative flex-1 overflow-auto scroll-area-hide">
          <div
            aria-hidden="true"
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                      {MODELS.options.map((model) => (
                        ...
                        >
                          <Check className={cn('mr-4 h-4 w-4', model.label === options.model.label ? "opacity-100" : "opacity-0")} />
                          {model.label}
                        </DropdownMenuItem>
                      ))}
                    </DropdownMenuContent>
                        ...
                        >
                          <span className="flex items-center">
                            <span className="flex flex-col text-sm">
                              <Label className="font-medium text-gray-900 mb-5">
                                {option.label}
                              </Label>
                              {option.description ? (
